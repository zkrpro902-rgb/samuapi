...
async function initDB(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS credentials (
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS rapports (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    type TEXT, lieu TEXT, auteur TEXT, membres TEXT,
    gravite TEXT, statut TEXT, description TEXT,
    discord_id TEXT, date TEXT
  )`).run();

  await db.prepare(`CREATE TABLE IF NOT EXISTS patients (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nom TEXT, motif TEXT, gravite TEXT, medecin TEXT,
    statut TEXT, notes TEXT, date TEXT
  )`).run();

  await db.prepare(`CREATE TABLE IF NOT EXISTS services (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    membre TEXT, grade TEXT, debut TEXT, fin TEXT, discord_id TEXT
  )`).run();

  await db.prepare(`CREATE TABLE IF NOT EXISTS membres (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    pseudo TEXT, grade TEXT, statut TEXT, discord_id TEXT,
    services INTEGER DEFAULT 0, warns INTEGER DEFAULT 0
  )`).run();

  await db.prepare(`INSERT OR IGNORE INTO credentials (username, password, role, discord_id)
    VALUES ('admin', 'samu2024', 'admin', NULL)`).run();
}
