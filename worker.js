...
async function initDB(db) {
  await db.exec(`
    ...
    INSERT OR IGNORE INTO credentials (username, password, role, discord_id)
    VALUES ('admin', 'samu2024', 'admin', NULL);
  `);
}
  ...
  async fetch(request, env) {
    ...
    if (path === "/services") {
      ...
      if (method === "POST") {
        ...
        ).bind(b.membre, b.grade, b.debut, b.fin || null, b.discord_id || null).run();
        return json({ id: result.meta.last_row_id, ...b });
      }
    }
    ...
    if (path === "/membres") {
      ...
      if (method === "POST") {
        ...
        ).bind(b.pseudo, b.grade, b.statut, b.discord_id || null).run();
        return json({ id: result.meta.last_row_id, ...b });
      }
    }
